...
import { getMyFarmerProduct } from '../../apis'
import AddIcon from '@mui/icons-material/Add'
...
export default function AgentProductsPage() {
  ...
  console.log(rows)
  const loadMyFarmerProduct = async () => {
    setLoading(true)
    const res = await getMyFarmerProduct(user._id)
  ...
  return (
    <Container>
      ...
      <AgentNavbar />
      <Box
        sx={{ display: 'flex', justifyContent: 'flex-end', margin: '1.3rem 0' }}
      >
        <Link to='/agent/add-product' style={{ textDecoration: 'none' }}>
          <Button variant='agent' startIcon={<AddIcon />}>
            Add Product
          </Button>
        </Link>
